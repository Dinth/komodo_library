# Patched copy of app/asr_models/mbain_whisperx_engine.py from
# onerahmet/openai-whisper-asr-webservice v1.10.0, bind-mounted over the stock
# file by r720-omv/mediarr/compose.yaml.
#
# The only change is the `if self.model is None` block in load_model().
# Upstream bug: https://github.com/ahmetoner/whisper-asr-webservice/issues/321
# release_model() (MODEL_IDLE_TIMEOUT) sets self.model = None, and load_model()
# then indexes into it, so every transcription after the first idle unload
...
# upstream file; once #321 is fixed upstream, delete it and the mount.
